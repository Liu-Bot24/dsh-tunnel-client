...
test('desktop packages support both latest npx and a validated installed DSH', () => {
  const packageJson = JSON.parse(fs.readFileSync(path.join(projectRoot, 'package.json'), 'utf8'))
  const macPackager = fs.readFileSync(path.join(projectRoot, 'scripts/package-mac.mjs'), 'utf8')
  const macRunner = fs.readFileSync(path.join(projectRoot, 'resources/dsh-runner/dsh'), 'utf8')
  const windowsRunner = fs.readFileSync(path.join(projectRoot, 'resources/dsh-runner/dsh.cmd'), 'utf8')
  ...
  assert.ok(packageJson.build.extraResources.some((entry) => entry.from === 'resources/dsh-runner' && entry.to === 'dsh-runner'))
  assert.match(macPackager, /resources', 'dsh-runner'/)
  assert.match(macRunner, /--yes --package=@deepseek-ai\/dsh@latest -- dsh/)
  assert.match(windowsRunner, /--yes --package=@deepseek-ai\/dsh@latest -- dsh/)
  assert.doesNotMatch(macRunner, /--yes @deepseek-ai\/dsh(?:\s|$)/)
  assert.doesNotMatch(windowsRunner, /--yes @deepseek-ai\/dsh(?:\s|$)/)
  assert.match(mainProcess, /'dsh-runner'/)
  assert.match(mainProcess, /resolveDshRuntime/)
  assert.match(mainProcess, /runtime\.environmentExecutable/)
  ...
  assert.match(mainProcess, /installed: inspectInstalledDshRuntime\(\)/)
  assert.match(renderer, /setRuntimeOptionDisabled\('system', busy \|\| !installedAvailable\)/)
  ...
  assert.match(mainProcess, /settings:save[\s\S]*return runLocalDshOperation/)
...
test('companion plugin actions refresh the real local DSH state before deciding whether installation is allowed', () => {
  ...
  assert.match(mainProcess, /companion-plugin:uninstall[\s\S]*companionPlugin\.uninstall\(await inspectCurrentLocalDsh\(\)\)/)
  assert.match(mainProcess, /commandEnvironment\(runtime\.environmentExecutable[\s\S]*localDsh = new LocalDshManager\([\s\S]*environment,/)
  assert.doesNotMatch(mainProcess, /companionPlugin\.(?:inspect|install)\(localDsh\.getState\(\)\)/)
  assert.match(renderer, /install\.disabled = companionPluginBusy \|\| !state\?\.canInstall/)
  assert.match(renderer, /uninstall\.disabled = companionPluginBusy \|\| !state\?\.canUninstall/)
  assert.match(mainProcess, /function localDshIsBusy[\s\S]*localDsh\?\.hasOwnedProcess\(\)/)
  assert.match(mainProcess, /settings:save[\s\S]*if \(localDshIsBusy\(state\)\)/)
  assert.match(mainProcess, /local-dsh:save[\s\S]*portChanged && localDshIsBusy\(\)/)
  assert.match(renderer, /Boolean\(localDshState\?\.owned\)/)
})
