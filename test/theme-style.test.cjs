...
test('desktop packages use the official npx entry as one automatic DSH path', () => {
  ...
  assert.match(macRunner, /--yes @deepseek-ai\/dsh "\$@"/)
  assert.match(windowsRunner, /--yes @deepseek-ai\/dsh %\*/)
  assert.doesNotMatch(macRunner, /--package|@latest/)
  assert.doesNotMatch(windowsRunner, /--package|@latest/)
