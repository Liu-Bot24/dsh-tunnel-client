...
test('adds the app-managed identity without invoking a shell', () => {
  const args = buildSshArgs({
    id: 'endpoint-1',
    name: 'Work Mac',
    sshHost: '192.0.2.10',
    ...
    remotePort: 3080,
    localPort: 13080,
  }, { identityFile: '/safe/app/id_ed25519' })
  assert.deepEqual(args.slice(-3), ['-i', '/safe/app/id_ed25519', 'test-user@192.0.2.10'])
})
