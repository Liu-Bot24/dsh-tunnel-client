...
// The small read script travels over stdin; the launch URL stays only in
// bounded process memory. macOS SSH shells may omit Homebrew from PATH.
...
`
  const runtimeNotFound = Symbol('runtime not found')
  const deadline = Date.now() + timeoutMs
  for (const runtime of ['node', '/opt/homebrew/bin/node', '/usr/local/bin/node']) {
    const remainingMs = deadline - Date.now()
    if (remainingMs <= 0) throw new Error('无法读取远端 DSH 认证信息')
    const result = await new Promise((resolve, reject) => {
      ...
      const timer = setTimeout(() => finish(new Error('timeout')), remainingMs)
      try {
        child = spawnProcess('ssh', [...args, runtime, '-'], { shell: false, windowsHide: true, stdio: ['pipe', 'pipe', 'pipe'] })
        ...
        child.once('close', code => {
          if (settled) return
          if (code === 127 && !body) return finish(null, runtimeNotFound)
    ...
    if (result !== runtimeNotFound) return result
  }
  throw new Error('无法读取远端 DSH 认证信息')
}
